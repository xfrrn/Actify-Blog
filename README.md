...
首页底部支持匿名提交痛点和选填的搜索词。内容先进入待审核列表，你在 `/admin/feedback` 输入管理密码后，可以公开、不公开或撤下反馈。

首次上线需要创建 Cloudflare D1 数据库、应用迁移并设置 `FEEDBACK_ADMIN_TOKEN`。配置、审核、导出和本地检查见 [反馈使用说明](docs/feedback.md)。
