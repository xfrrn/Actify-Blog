...
每条内容有 `data`（草稿）、`published`（线上快照）和递增 `version`。保存、发布均核对版本，409 冲突不覆盖新内容。每次发布只替换所选语言。撤回后其他已发布译文仍可访问；全部撤回后详情与分享图返回 404，列表、RSS、站点地图移除该地址。回收站撤下所有译文；恢复只回到草稿。首次发布后地址锁定，不提供永久删除或完整历史记录。
...
```text
DATA_DIR/
  actify.sqlite        内容、反馈、密码哈希、会话、导入记录
  actify.sqlite-wal    SQLite WAL，运行中可能存在
  actify.sqlite-shm
  uploads/             本地开发图片与切换前的旧素材
```

生产目录放在代码外，例如 `/var/lib/actify`，仅运行用户可读写。不放在 public、Nginx 静态目录或临时容器磁盘中。使用本机持久化磁盘，只运行一个 Node 服务实例。开发默认 `.data/`。CLI 读取 `.env.local` / `.env`，进程环境变量优先。

线上新图片存储在 R2，SQLite 保留文件名、尺寸、类型、存储位置和永久公开 URL。素材库、封面和正文直接使用这个 URL。旧本地记录自动升级并继续从 `/media/` 读取，不迁移、不删除已有文件。
...
请求路径：浏览器 → Cloudflare → Nginx HTTPS → 127.0.0.1:3000 → Next.js / SQLite。准备 Node.js 24、pnpm 10.33.3、Nginx 和域名有效证书。Node 可执行路径不同则调整 systemd 的 ExecStart。
...
sudo -u actify env DATA_DIR=/var/lib/actify pnpm content:import
sudo -u actify env DATA_DIR=/var/lib/actify pnpm admin:password
sudo -u actify env DATA_DIR=/var/lib/actify SITE_ORIGIN=https://actify.cc pnpm build
...
[systemd 单元](../deploy/actify.service)已包含生产默认值。在 `/etc/actify.env` 写入 R2 配置及下面的环境值，设置所有者 `root:actify`、权限 `640`，让服务及 actify 用户运行的 CLI 可读取：
...
备份包含一致 SQLite 快照、快照引用的全部上传图片、SHA-256 清单。R2 图片通过带签名的 S3 GET 下载进入备份，旧本地图片直接复制；任何下载失败都不会写出成功清单。可在线执行，R2 文件需保持不删除、不覆盖。备份输出必须为 DATA_DIR 外一个不存在的新目录，父目录先创建。
...
恢复验证哈希、SQLite 完整性、图片引用，并清空会话。R2 素材恢复到原桶和原公开域名：缺失对象会重新上传；已有对象逐字节一致时复用，不一致则报错，绝不覆盖。需要原桶的读写凭据，签名密钥不包含在备份中。保持原图片域名有效，正文里的公开 URL 才继续可用。只允许不存在或空 DATA_DIR，绝不覆盖在线库：

```sh
sudo systemctl stop actify
cd /opt/actify
sudo -u actify env DATA_DIR=/var/backups/actify/restored-data /usr/bin/node --env-file=/etc/actify.env scripts/admin.mjs restore /var/backups/actify/manual-2026-09-21
# 修改 /etc/actify.env：DATA_DIR=/var/backups/actify/restored-data
sudo systemctl start actify
```

该示例将恢复目录直接作为新持久化目录；后续备份另选 DATA_DIR 之外的新目录。保留旧库，核对文章、译文、作品、反馈、图片，重启后再检查。失败可停服务切回旧 DATA_DIR。不要只复制运行中的 actify.sqlite 而遗漏 WAL。

代码升级：先备份，停止服务，更新代码和依赖并构建，启动服务。保留 DATA_DIR，不从仓库覆盖后台内容。日志：`journalctl -u actify -n 100`。
...
核心测试实际备份、恢复，核对内容、图片并拒绝损坏备份。R2 测试使用假凭据和模拟传输，核对 S3 签名、上传失败、旧库升级、混合备份与不覆盖恢复。HTTP 测试启动隔离的生产服务，验证权限、来源、动态发布、译文、RSS / sitemap / 分享图、作品、媒体、审核，重启后再读内容与图片。

完整站点只读检查使用与运行服务相同的 DATA_DIR：

```sh
DATA_DIR=/var/lib/actify node scripts/check-site.mjs http://127.0.0.1:3000
```
