...
src/lib/cms-*.ts          PostgreSQL 连接池、模型、版本检查、导入
...
内容流：content 或旧 SQLite → 一次性导入 → DATABASE_URL 指定的 PostgreSQL。后台在事务内锁定内容行并核对版本，保存 JSONB data，显式发布复制到 published；前台、RSS、sitemap、分享图实时读取。图片校验、重新编码后上传 R2 的 media/{uuid}.{ext}，PostgreSQL 记录公开域名链接；本地开发和旧素材保留 DATA_DIR/uploads 与 /media/ 路由。
