...
try {
  ...
  assert.deepEqual(tables.rows.map(({ tablename }) => tablename), ["content", "feedback", "media", "rate_limits", "sessions"], "Startup creates all tables before any request");
  ...
  const loginPage = await (await request("/admin/login", "GET", undefined, false)).text();
  assert.match(loginPage, /name="username"/); assert.doesNotMatch(loginPage, new RegExp(`${username}|${password}`));
  assert.equal((await request("/api/admin/login", "POST", { username, password }, false, { Origin: "https://attacker.invalid" })).status, 403);
  assert.equal((await request("/api/admin/login", "POST", { password }, false)).status, 400);
  const wrongPassword = await json(await request("/api/admin/login", "POST", { username, password: "wrong" }, false), 401);
  assert.deepEqual(await json(await request("/api/admin/login", "POST", { username: "wrong", password }, false), 401), wrongPassword);
  for (let i = 0; i < 7; i++) assert.equal((await request("/api/admin/login", "POST", { username, password: "wrong" }, false)).status, 401);
  const limited = await request("/api/admin/login", "POST", { username, password }, false, { "X-Real-IP": "203.0.113.2", "X-Forwarded-For": "203.0.113.3" });
  assert.equal(limited.status, 429, "A correct password and spoofed IP headers cannot bypass the login limit");
  assert.ok(Number(limited.headers.get("retry-after")) > 0 && Number(limited.headers.get("retry-after")) <= 600);
  await probe.query("UPDATE rate_limits SET expires=0 WHERE key='login:direct'");
  await probe.query("UPDATE rate_limits SET count=100 WHERE key='login:global'");
  assert.equal((await request("/api/admin/login", "POST", { username, password }, false)).status, 429, "Global login limit covers requests across IPs");
  await probe.query("UPDATE rate_limits SET expires=0 WHERE key LIKE 'login:%'");
  const login = await request("/api/admin/login", "POST", { username, password }, false); assert.equal(login.status, 200);
  ...
  await probe.query("UPDATE rate_limits SET count=20 WHERE key='admin:upload'");
  assert.equal((await fetch(`${origin}/api/admin/media`, { method: "POST", headers: { Origin: origin, Cookie: cookie, "Content-Type": "image/png" }, body: image })).status, 429);
  await probe.query("UPDATE rate_limits SET expires=0 WHERE key='admin:upload'");
  await probe.query("UPDATE rate_limits SET count=120 WHERE key='admin:write'");
  const writeLimit = await request("/api/admin/posts", "POST"); assert.equal(writeLimit.status, 429);
  assert.ok(Number(writeLimit.headers.get("retry-after")) > 0 && Number(writeLimit.headers.get("retry-after")) <= 60);
  assert.equal((await request("/api/admin/posts")).status, 200, "Write limits do not block reading drafts");
  await probe.query("UPDATE rate_limits SET expires=0 WHERE key='admin:write'");
  ...
  for (let i = 0; i < 21; i++) await json(await request("/api/admin/posts", "POST"), 201);
  ...
  await probe.query("INSERT INTO rate_limits(key,count,expires) VALUES ('admin:write',120,$1) ON CONFLICT(key) DO UPDATE SET count=120, expires=EXCLUDED.expires", [Date.now() + 60000]);
  assert.equal((await request("/api/admin/posts", "POST")).status, 429);
